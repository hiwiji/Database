...
-- 			 ADD [CONSTARAINT 제약조건명] 제약조건 (지정할 컬럼명)
...
-- 복사한 테이블 조회 (기존과 내용 같은지)
SELECT * FROM DEPT_COPY;

-- DEPT_COPY의 DEPT_TITLE 컬럼에 UNIQUE 제약조건 추가
ALTER TABLE DEPT_COPY 
ADD CONSTRAINT DEPT_COPY_TITLE_U UNIQUE(DEPT_TITLE);

-- DEPT_COPY의 DEPT_TITLE 컬럼에 UNIQUE 제약조건 삭제
ALTER TABLE DEPT_COPY
DROP CONSTRAINT DEPT_COPY_TITLE_U;  
  --> 제약조건추가삭제하면 PROPERTIES에 CONSTRAINTS에 추가삭제됨
...
-- 컬럼 자체에 NULL 허용/비허용을 제어하는 성질의 형태로 인식됨.
...
-- 다시조회
SELECT * FROM DEPT_COPY;
--> 컬럼이 생서되면서 DEFAULT 값이 자동 삽입됨.


-- DEPT_COPY 테이블 안 DEPT_ID 컬럼에  D10 개발1팀 추가 
INSERT INTO DEPT_COPY 
VALUES('D10', '개발1팀', 'L1', DEFAULT, DEFAULT);
-- ORA-12899: "KH"."DEPT_COPY"."DEPT_ID" 열에 대한 값이 너무 큼(실제: 3, 최대값: 2)

-- DEPT_ID 컬럼 수정
ALTER TABLE DEPT_COPY MODIFY DEPT_ID VARCHAR2(3); --TYPE :CHAR(2) 가 VARCHAR2(3)으로 변경

INSERT INTO DEPT_COPY      -- 다시 개발1팀 추가 
SELECT * FROM DEPT_COPY;   -- 테이블 안에 D10 추가됨


-- LNAME의 기본값을 'KOREA'로 수정
ALTER TABLE DEPT_COPY MODIFY LNAME DEPART 'KOREA';
SELECT * FROM DEPT_COPY;
...
SELECT * FROM DEPT_COPY; -- DROP 확인

-- 테이블 삭제
DROP TABLE DEPT_COPY;  -- 알림창 뜨면 확인 누르기

SELECT * FROM DEPT_COPY;   -- 테이블 삭제되서 아래 오류구문 뜸 
...
-- TB2DHK TB1이 관계가 형성되있어서 그런거임

--> 해결방법
-- 1) 자식, 부모 테이블 순서로 삭제 (TB2 지우고 TB1 지움)
-- 2) ALTER를 이용해서 FK제약조건 삭제 후 TB1 삭제
-- 3) DROP TABLE 삭제옵션 CASCADE CONSTRAINTS 사용 
--> CASCADE CONSTRAINTS : 삭제하려는 테이블과 연결된 FK제약조건 모두 삭제 

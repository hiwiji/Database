...
-- DQL (Data Query Language) : 데이터 질의(조회) 언어   ex. SELECT은 기본은 dql이지만 dml도 같이

-- DML (Data Manipulation Language) : 데이터 조작 언어. 테이블에 있는 데이터를 삽입 ,수정, 삭제 하는 언어
...
-- 테이블이나 인덱스, 뷰 등 << 다양한 데이터베이스 객체를 생성하는 구문 >>   <>정도는 꼭 시험에 적기 
 ...
 * NUMBER : 숫자형(정수, 실수) 
 * 
 * CHAR(크기) : 고정길이 문자형( 2000 BYTE까지만 가능 )
 *  --> ex) char(10) 컬럼에 'ABC' 3BYTE 문자열만 저장해도 이미 10BYTE 저장공간을 모두 사용.
 * 
 * VARCHAR2(크기) : 가변길이 문자형 ( 4000 BYTE까지만 가능)
 *  --> ex) VARCHAR2(10) 컬럼에 'ABC' 3BYTE 문자열만 저장하면 나머지 7 BYTE를 반환함.(알아서 줄여준다)
 * 
 * DATE : 날짜 타입
 * 
 * BLOB : 대용량 이진 데이터 (4GB)  EX) 동영상, 사진 파일같은거 
 * CLOB : 대용량 문자 데이터 (4GB)
 *  
 * 1BYTE * 1024 = 1KB
 * 1KB * 1024 =  1MB
...
-- 만든 테이블 확인
SELECT * FROM MEMBER; -- 테이블 명에 ""(쌍따옴표) 붙여도 테이블 만들 수 있다. 
-- SELECT * FROM "MEMBER"
...
INSERT INTO MEMBER VALUES('MEM01', '12345ABC', '홍길동', '991213-1234567', DEFAULT);
																			-- DEFAULT라고 쓰면 SYSDATE값 나옴 													

-- INSERT/UPDATE 시 컬럼값으로 DEFAULT를 작성하면
-- 테이블 생성시 해당 컬럼에 지정된 DEFAULT값으로 삽입이 된다.

SELECT * FROM MEMBER;

-- 추가 샘플 데이터 삽입
INSERT INTO MEMBER VALUES('MEM02', '23456ABC', '김덕근', '951234-1234567', SYSDATE);
INSERT INTO MEMBER VALUES('MEM03', '34567ABC', '김수현', '967478-1378945', DEFAULT);
-- 여기서는 전체데이터값을 넣어줘야해서 DEFAULT 내용을 써줘야함

COMMIT;

-- INSERT시 미작성 하는 경우에(가입일) DEFAULT 값이 반영됨 
INSERT INTO MEMBER (MEMBER_ID, MEMBER_PWD, MEMBER_NAME)
VALUES('MEM04', '97524ABC', '이지연');

SELECT * FROM MEMBER;

-- ** JDBC에서 날짜를 입력 받았을 때 삽입하는 방법 **
-- '2023-01-13 10:33:27'
INSERT INTO MEMBER VALUES('MEM05', 'PASS05', '김준수', '121212-1234576',
...
-- 1. NOT NULL
-- 해당 컬럼에 반드시 값이 기록되어야하는 경우에 사용
-- 삽입/수정시 NULL값을 허용하지 않도록 컬럼레벨에서 제한

-- * 컬럼레벨? : 테이블 생성 시 컬럼을 정의하는 부분 작성하는 것
...
INSERT INTO USER_USED_PK
VALUES(NULL, 'user03', 'pass03', '고길순', '여', '010-4566-5678', 'test@kh.or.kr');
--> 기본키가 NULL이므로 오류
...
-- 컬럼레벨일 경우
-- 컬럼명 자료형(크기) [CONSTARAINT 이름] REFERENCES 참조할테이블명 [(참조할컬럼)] [삭제롤]
...
INSERT INTO USER_USED_FK
VALUES(4, 'user04', 'pass04', '안중군', '남', '010-2222-2222', 'ahn123@test.com', NULL);
...
INSERT INTO USER_USED_FK
VALUES(5, 'user05', 'pass05', '윤봉길', '남', '010-5555-2222', 'ahn123@test.com', 50);
...
SELECT * FROM USER_USED_FK3;   -- 홍길동이랑 이순신 사라짐 
...
SELECTTO_CHAR(TO_DATE('210505', 'YYYY/MM/DD'), 'YYYY"년" MM"월" DD"일"')
FROM DUAL;


SELECT EMP_NAME AS 직원명, JOB_CODE AS 직급코드, SALARY AS 급여,
SALARY + (SALARY*DECODE(JOB_CODE, 'J7', 0.08, 'J6', 0.07, 'J5', 0.05, 0.03)) AS 인상급여
FROM EMPLOYEE;
...
SELECT
TO_CHAR(TO_DATE('210505', 'YYYYMMDD'), ' YYYY"년" MM"월" DD"일" ')
FROM DUAL;
...
SELECT EMP_NAME 직원명, JOB_CODE 직급코드, SALARY 급여, 
DECODE(JOB_CODE, 'J7' , '8%' , 'J6', '7%', 'J5', '5%' , '3%') 인상급여
FROM EMPLOYEE ;
...
ALTER TABLE EMOLOYEE_COPY ADD CONSTRAINT PK_EMP_COPY PRIMARY KEY(EMP_ID);

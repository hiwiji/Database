 ...
 * 
 * - RESULT SET은 0개 이상의 행을 포함할 수 있따.
...
-- SYSDATE -1  : 2023-01-08
-- SYSDATE     : 2023-01-09
-- SYSTATE +1  : 2023-01-10
...
-- SELECT 조회 결과의 집합임 RESULT SET에 출력되는 컬럼명을 지정
...
-- EMPLOYEE 테이블에서 [[급여가 3백만원 초과인 사원의]] []는 조건임
-- 사번, 이름, 급여, 부서코드를 조회
...
-- '_' 예시   (사용 할 때는 __ 붙여서 사용하기!!!!)
-- 'A_' : A로 시작하는 두 글자 문자열
-- '_ _ _A' : A로 끝나는 네 글자 문자열
-- '_ _A_ _' : 세 번째 문자가 A인 다섯 글자 문자열
-- '_ _ _ A _' : 네 번째 문자가 A인 다섯 글자 문자열
-- '_ _ _ _ _' : 다섯 글자 문자열  
...
-- 언더바를 사용할 수도 있지만 이름이 외자거나 네자일 수도 있어서 %를 쓰는게 더 좋음!
...
FROM EMPLOYEE
ORDER BY SALARY;  -- ASC가 기본값이라 오름차순
-- WHERE절은 조건 없어서 안써줘도 됨 
...
FROM EMPLOYEE
ORDER BY 입사일;
--SELECT절에서 별칭을 먼저 정해주고 ORDER BY에서 찾아줘서 검색되는것임

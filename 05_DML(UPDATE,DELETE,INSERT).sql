...
SELECT * FROM EMPLOYEE2;  -- 복제본 잘 만들어졌는지 확인차 쓴 구문
...
-- 1) INSERT INTO 삽입할 테이블명 VALUES( 데이터, 데이터, 데이터..... )
...
-- DEPARTMENT2 테이블에서 DEPT_ID가 'D9'인 부서 정보 조회 (조회니까 SELECT문)
SELECT * FROM DEPARTMENT2
...
ROLLBACK;  -- 롤백 실행하면 다시 장채현 돌아옴


-- EMPLOYEE2 테이블 전체 행 삭제
SELECT * FROM EMPLOYEE2;

DELETE FROM EMPLOYEE2; --> 24개 행 전체 삭제 (WHERE절 안써서)

ROLLBACK;  --다시 원래대로 돌리기~~ 24행 장채현 나옴

-------------------------------------------DELETE 끝  ------------------------------------------------------------

-- 5. TRUNCATE (DDL입니다. DML 아님)
-- 테이블의 전체 행을 삭제하는 DDL
-- DELETE보다 수행속도가 더 빠르다.
-- DELETE와 가장 큰 차이점 : ROLLBACK을 통해 복구할 수 없다.

...
FROM EMPLOYEE
JOIN DEPARTMENT ON(DEPT_CODE = DEPT_ID) ;   -- 등가조인(오라클) OR 이너조인(안시)
...
-- EMPLOYEE 테이블, DEPARTMENT 테이블을 참조하여             // DEPARTMENT테이블
-- 사번(EMP_ID), 이름(EMP_NAME), 부서코드(DEPT_CODE), 부서명(DEPT_TITLE) 조회
...
FROM DEPARTMENT -- 테이블을 적을 때 하나의 기준을 잡아야 한다. 이건 DEPARTMENT로 잡음
JOIN LOCATION ON (LOCATION_ID = LOCAL_CODE);   -- ()안에 DEPARTMENT 먼저 써줌
...
-- 사번(EMP_ID), 이름(EMP_NAME), 직급코드(JOB_CODE), 직급명(JOB_NAME) 조회
     -- 사번, 이름, 직급코드 다 EMPLOYEE 테이블   / 직급명은 JOB 테이블 
...
-- ANSI 표준
-- 사원의 이름(EMP_NAME)과, 부서명(DEPT_TITLE) 조회
SELECT EMP_NAME, DEPT_TITLE
...
--  조인되는 테이블의 각 행들이 모두 매핑된 데이터가 검색되는 방법(곱집합) // 모든 경우의 수 출력
...
-- 사번(EMP_ID), 이름(EMP_NAME), 사수의 사번(MANAGER_ID), 사수 이름 조회
...
-- 사원이름(EMP_NAME), 부서명(DEPT_TITLE), 지역명 조회(LOCAL_NAME)
...
AND LOCATION_ID = LOCAL_CODE; -- (EMPLOYEE + DEAPRTMENT) + LOCATION에 조인
...
-- 사번(EMP_ID), 이름(EMP_NAME), 직급명(JOB_NAME), 부서명(DEPT_TITLE), 근무지역명(LOCAL_NAME), 급여(SALARY)를 조회하세요
--  EMPLOYEE   ,  EMPLOYEE   ,   JOB  , DEPARTMENT, LOCATION, EMPLOYEE

-- EMPLOYEE , JOB, DEPARTMENT, LOCATION 4개 테이블 사용 

-- ANSI

SELECT EMP_ID, EMP_NAME, JOB_NAME, DEPT_TITLE, LOCAL_NAME, SALARY
FROM EMPLOYEE
JOIN JOB USING(JOB_CODE)  -- EMPLOYEE테이블 JOB_CODE와 JOB테이블 JOB_CODE와 동일하여 USING 사용
JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
...
-- 사원명(EMP_NAME), 주민번호(EMP_NO), 부서명(DEPT_TITLE), 직급명(JOB_NAME)을 조회하시오.
--     EMPLOYEE 			EMPLOYEE			DEPARTMENT			JOB	
...
-- 2. 이름에 '형'자가 들어가는 직원들의 사번(EMP_ID), 사원명(EMP_NAME), 직급명(JOB_NAME), 부서명(DEPT_TITLE)을 조회하시오.
-- 											EMPLOYEE			EMPLOYEE		JOB					DEPARTMENT
...
-- 3. 해외영업 1부, 2부 에 근무하는 사원의 
-- 사원명(EMP_NAME), 직급명(JOB_NAME), 부서코드(DEPT_CODE), 부서명(DEPT_TITLE)을 조회하시오.
--		EMPLOYEE		JOB			DEPARTMENT			DEPARTMENT
...
--4. 보너스포인트를 받는 직원들의 
--사원명(EMP_NAME), 보너스포인트(BONUS), 부서명(DEPT_TITLE), 근무지역명(LOCAL_NAME)을 조회하시오.
-- EMPLOYEE				 EMPLOYEE				DEPARTMENT		 LOCATION
...
--5. 부서가 있는 사원의 사원명(EMP_NAME), 직급명(JOB_NAME), 부서명(DEPT_TITLE), 지역명(LOCAL_NAME) 조회

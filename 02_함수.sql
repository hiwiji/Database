...
-- INSTR(컬럼명 | 문자열, '찾을 문자열' [, 찾기 시작할 위치 [, 순번]) // [대괄호는 안써도 무방]
-- 지정한 위치부터 지정한 순번째로 검색되는 문자의 위치를 반환

-- 문자열을 앞에서부터 검색하여 첫 번쨰 B 위치를 조회
SELECT INSTR('AABAACAABBAA', 'B') FROM DUAL; -- 결과값 3

-- 문자열을 5번째 문자부터 검색하여 첫번째 B 위치 조회
SELECT INSTR('AABAACAABBAA', 'B', 5) FROM DUAL;  -- 결과값 9

-- 문자열을 5번째 문자부터 검색하여 두번째 B 위치 조회
SELECT INSTR('AABAACAABBAA', 'B', 5, 2) FROM DUAL; -- 결과값 10
...
-- ABS(숫자 | 컬럼명) : 절대 값
SELECT ABS(10), ABS(-10) FROM DUAL;  -- 결과값 10
...
FROM EMPLOYEE
WHERE MOD(EMP_ID, 2) <> 0;  
-- DB에서는 != 나 <> 같은 뜻이지만(같지않다) 자동으로 <> 변환해줘서 
-- 같지 않을 경우에는 <> 로 쓰는게 더 좋다.




-- ROUND(숫자 | 컬럼명[,소수점 위치]) : 반올림
SELECT ROUND(123.456) FROM DUAL; -- 소수점 첫번째 자리에서 반올림      // 결과값 123

SELECT ROUND(123.456, 1) FROM DUAL; --소수점 두번째 자리에서 반올림  
                                 --> 두번째 자리에서 반올림해서 소수점 한자리까지만 표현 // 결과값 123.5

SELECT ROUND(123.456, 0) FROM DUAL; -- 소수점 첫번째 자리에서 반올림 (0 기본값) // 결과값 123

SELECT ROUND(123.456, -1) FROM DUAL; -- 소수점 0번째 자리에서 반올림 해서 
                                 --> 소수점 -1 자리 표현
                                 --> == 1의 자리에서 반올림해서 10의 자리부터 표현 // 결과값 120
...
SELECT CEIL(123.1), FLOOR(123.9) FROM DUAL; -- 결과값 CEIL 124, FLOOR 123




-- TRUNC(숫자 | 컬럼명 [,위치]) : 특정 위치 아래를 버림(절삭)
SELECT TRUNC(123.456) FROM DUAL; -- 소수점 아래를 버림 (기본값) // 결과값 123
SELECT TRUNC(123.456, 1) FROM DUAL; -- 소수점 첫째자리 아래를 버림 // 결과값 123.4
SELECT TRUNC(123.456, -1) FROM DUAL;-- 10의 자리 아래 버림 // 결과값 120
...
SELECT SYSDATE FROM DUAL;   -- // 결과값 : 2023-01-09 14:17:17.000


-- SYSTIMESTAMP : SYSDATE + MS 단위 추가 
SELECT SYSTIMESTAMP FROM DUAL; --// 결과값 : 2023-01-09 14:17:56.174 +0900
...
SELECT EMP_NAME,
 	...
 	EXTRACT(MONTH FROM HIRE_DATE) || '일' AS 입사일
 FROM EMPLOYEE;
...
-- L : 현재 DB에 설정된 나라의 화폐 기호 (원화표시 ￦)

SELECT TO_CHAR (1234, '99999') FROM DUAL; -- 결과값 ' 1234'
SELECT TO_CHAR (1234, '00000') FROM DUAL; -- 결과값 '01234'

SELECT TO_CHAR (1000000, 'L9,999,999') FROM DUAL; -- 결과값 ￦1,000,000 

SELECT TO_CHAR (1000000, '$9,999,999') FROM DUAL; -- 결과값 $1,000,000
...
SELECT SYSDATE FROM DUAL; -- 결과값 2023-01-10 10:21:19.000
...
-- NVL (컬럼명, 컬럼값이 NULL 일 때 바꿀 값 ) : NULL인 컬럼 값을 다른 값으로 변경
...
-- 직원의 급여를 인상하고자 한다.
-- 직급 코드(JOB_CODE) 가 J7인 직원은 20% 인상
-- 직급 코드(JOB_CODE) 가 J6인 직원은 15% 인상
-- 직급 코드(JOB_CODE) 가 J5인 직원은 10% 인상
-- 그 외 직급은 5% 인상
-- 이름, 직급코드, 원래급여, 인상률, 인상된 급여
...
-- CASE END 문법 (자바에서 SWITCH ELSE IF 같은거 )
...
-- 이름(EMP_NAME), 급여(SALARY), 직급코드(JOB_CODE) 조회
...
-- BONUS를 받는 사원수
SELECT COUNT(BONUS) FROM EMPLOYEE; -- 9개

SELECT COUNT(*) FROM EMPLOYEE
WHERE BONUS IS NOT NULL; -- 9개

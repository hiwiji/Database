...
-- OR REPLACE 추가 후 실행 -> 성공 확인되면 조회해보기
...
-- 복사한 DEPT_COPY2 테이블을 이용해서 VIEW 생성
SELECT * FROM DEPT_COPY2;   --> 복사 잘 되었는지 확인
...
--> VIEW에 삽입한 내용이 원본 테이블에 존재하는걸 확인
--> VIEW를 이용한 DML 구문은 원본에 영향을 미친다.

-- VIEW를 이용한 DML시 발생하는 문제점 == 제약조건 위배 현상
ROLLBACK;
SELECT * FROM DEPT_COPY2;  -- 롤백 잘 되었는지 확인
SELECT * FROM V_DCOPY2;   -- 롤백 잘 되었는지 확인
...
WITH READ ONLY; -- 읽기전용 옵션 (SELECT만 가능)

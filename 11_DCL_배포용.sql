/*
	...
	DCL의 사전적 의미  작성법보다 개념위주로 보세요
    - 계정의 DB, DB 객체에 대한 접근 권한을
...
CREATE USER wje_sample IDENTIFIED BY sample1234;


-- 2. 새 연결(접속방법) 추가
--> ORA-01045 : 사용자 WJE_SAMPLE는 CREATE SESSION 권한 --> 접속권한
--을 가지고 있지 않음. 로그온이 거절되었습니다.


-- 3. 접속 권한 부여
-- [권한 부여 작성법]
-- GRANT 권한, 권한, 권한...... TO 사용자명;
GRANT CREATE SESSION TO wje_sample;   

-- 파일 밑 코드 모양 눌러서 오라클 클릭 후 database 부분 XE 적기
-- USERNAME 에 아이디(wje_sample) 비밀번호(sample1234) 입력후 test connection 누르고
-- 버전명 뜨면 접속권한 부여 된거임
...
GRANT CREATE TABLE TO wje_sample;

ALTER USER wje_sample DEFAULT TABLESPACE
SYSTEM QUOTA UNLIMITED ON SYSTEM; -- 제한없이 공간 할당 해준다는 내용
...
GRANT CONNECT, RESOURCE TO wje_sample;
...
-- kh / wje_sample 사용자 계정끼리 서로 객체 접근 권한 부여
...
-- 2. (kh) wje_sample 계정에 EMPLOYEE 테이블 조회 권한 부여

-- [객체 권한 부여 방법]
-- GRANT 객체권한 ON 객체명 TO 사용자명;
GRANT SELECT ON EMPLOYEE TO wje_sample;


-- 3. (sample) 다시 kh.EMPLOYEE 조회
SELECT * FROM kh.EMPLOYEE;
-- SELECT * FROM kh.DEPARTMENT; --> DEPARTMENT 테이블 조회권한은 준 적 없으므로 불가!
...
REVOKE SELECT ON EMPLOYEE FROM wje_sample;

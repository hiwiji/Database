...
WHERE SALARY > (/*단일행 서브쿼리 자리*/ SELECT AVG(SALARY) FROM EMPLOYEE)
                                  -- // 서브쿼리 자리에 결과값이 하나만 나와서 단일행 서브쿼리임
                                  -- // 헷갈리면 서브쿼리 문구만 따로 떼와서 조회하면 한개만 나옴 
ORDER BY JOB_CODE;
...
AND SALARY < ANY (SELECT SALARY
			  ...
			  NATURAL JOIN JOB
			  WHERE JOB_NAME = '과장');		
-- > ANY : 과장급여 3개 중, 최소금액인 220보다 더 큰 SALARY
...
-- 퇴사한 여직원(ENT_DATE)과 같은 부서, 같은 직급에 해당하는
...
--    사번(EMP_ID), 이름(EMP_NAME), 부서코드(DEPT_CODE), 직급코드(JOB_CODE), 
--    부서명(DEPT_TITLE), 직급명(JOB_NAME)
...
-- 8. RANK() OVER / DENSE_RANK() OVER         // 시험에 나옴!!

-- RANK() OVER : 동일한 순위 이후의 등수를
--               동일한 인원수 만큼 건너뛰고 순위 계산.
-- EX) 1위가 두명이면 다음 순위는 3위로 뜨게 하는것
-- EX) 공동1위가 2명이면, 다음 순위는 2위가 아니라 3위
...
FROM (SELECT EMP_NAME, SALARY
	  FROM EMPLOYEE
	  ORDER BY SALARY DESC 
);   -- 19위 전형돈/ 20위 윤은해, 21위 박나라 둘다 급여 같은데 숫자가 다름
...
-- DESC RANK() OVER : 동일한 순위 이후에 등수를 이후의 순위로 계산
